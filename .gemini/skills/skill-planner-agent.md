...
3. **Test-Case Plan by Type** (this is the primary deliverable):
   ...
   - **Integration** (`@integration`): Multi-step flows that chain dependent endpoints together (the DAG from step 1), verifying data flows correctly across calls. **Requirement**: Single clean test case title, using internal `test.step('1. ...')` sub-steps for each chained endpoint call, specifying passed state (IDs/tokens) and per-step assertion checkpoints.
   ...
   - **E2E** (`@e2e`): A realistic end-to-end user journey spanning multiple domains, only where the spec supports a coherent journey (e.g. signup → login → create resource → use it → clean up). **Requirement**: Single clean test case title, using internal `test.step('1. ...')` sub-steps for each multi-domain step, specifying prerequisites, state transitions, and per-step acceptance criteria.
